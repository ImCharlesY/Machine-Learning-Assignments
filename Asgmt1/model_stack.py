...
train_x_stack = stacking_train(classifiers[0], classifiers[1])
start_time = dt.datetime.now()
print('Start learning test dataset stacking at {0}.'.format(str(start_time)))
test_x_stack = stacking_test(classifiers[0], classifiers[1])

# Start fitting training dataset with the third classifier
start_time = dt.datetime.now()
print('Startfitting training dataset with the third classifier {0} at {1}.'.format(type(classifiers[2]).__name__, str(start_time)))
classifiers[2].fit(train_x_stack, train_y)
...
predicted = classifiers[2].predict(test_x_stack)
print('-' * 50)
print('Classification report for classifier %s:\n%s\n'
      % (classifiers[2], metrics.classification_report(expected, predicted)))
